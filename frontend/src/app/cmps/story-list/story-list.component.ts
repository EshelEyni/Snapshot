...
export class StoryListComponent implements OnInit, OnChanges, OnDestroy {
  ...
  ngOnInit(): void {
    this.loggedinUserSub = this.loggedinUser$.subscribe((user) => {
      if (user) {
        ...
        this.storyService.loadStories(this.type);
      }
    });
    ...
    if (this.type !== 'profile-details') {
      this.storySub = this.storyService.stories$.subscribe((stories) => {
        ...
        switch (this.type) {
          case 'story-details':
            this.preCurrStoryList = stories.slice(
              0,
              stories.findIndex((s) => s.id === this.currStory.id)
            );
            this.postCurrStoryList = stories.slice(
              stories.findIndex((s) => s.id === this.currStory.id) + 1
            );
            break;
      ...
      this.storySub = this.storyService.highlightedStories$.subscribe(
        (stories) => {
          this.stories = stories;
          this.setPaginationBtns(stories);
        }
  ...
  ngOnChanges(): void {
    if (this.currStory && this.stories && this.preCurrStoryList) {
      this.preCurrStoryList = this.stories.slice(
        0,
        this.stories.findIndex((story) => story.id === this.currStory.id)
      );
      this.postCurrStoryList = this.stories.slice(
        this.stories.findIndex((story) => story.id === this.currStory.id) + 1
      );
  ...
  setPaginationBtns(stories: Story[]): void {
    if (this.type === 'story-details') {
      if (this.preCurrStoryList.length === 0)
        this.isPaginationBtnShown.left = false;
      else this.isPaginationBtnShown.left = true;

      if (!this.postCurrStoryList.length)
      ...
      if (this.idx === stories.length - 4 && window.innerWidth > 400)
        this.isPaginationBtnShown.right = false;
      else this.isPaginationBtnShown.right = true;

      if (this.idx === stories.length - 3 && window.innerWidth < 400)
        this.isPaginationBtnShown.right = false;
      else this.isPaginationBtnShown.right = true;
    }
  }
  ...
  onPickStoryForHighlights(idx: number): void {
    if (!this.isStorySelectedForHighlight[idx].isSelected) {
      this.isStorySelectedForHighlight.forEach(
        (story) => (story.isSelected = false)
      );
